$(function() {
	...
	function playPoker(index, cancel_num) { //index:1-3,cancel_num:连续要不起的次数
		//		console.log(game)
		//初始化
		$('.play_btn2').hide();
		$('.play').off('click', 'li');
	...
	function getHint(index) {
		...
		switch(desk_type) {
			...
			case 1:
				...
				break;
			case 2:
				if(player_poker.length < 2) break;
				...
				break;
			case 3:
				if(player_poker.length < 3) break;
				...
				break;
			case 4:
				if(player_poker.length < 4) break;
				for(let i = 0; i < player_poker.length - 2; i++) {
					if(player_poker[i].num > desk_max && player_poker[i].num == player_poker[i + 2].num) {
						sel_poker.push(player_poker[i], player_poker[i + 1], player_poker[i + 2]);
						if(i == 0) {
				...
				break;
			case 5:
				if(player_poker.length < desk_poker.length) break;
				for(let i = 0; i < player_poker.length; i++) {
	...
	function win(index) {
		...
		if(player[index - 1].role == 1) {
			// console.log(index);
			for(let i = 0; i < 3; i++) {
				player[i].integral -= mul;
				$('tr').eq(2).children().eq(i + 1).html('-' + mul);
			}
			player[index - 1].integral += 3 * mul;
			$('tr').eq(2).children().eq(index).html('+' + mul * 2)
		} else {
			for(let i = 0; i < 3; i++) {
				...
				if(player[i].role == 1) {
					...
					player[i].integral += mul / 2;
					$('tr').eq(2).children().eq(i + 1).html('+' + mul / 2)
				}
			}
		...
		//玩家结束积分
		$('tr').eq(3).children().eq(1).html(player[0].integral);
		$('tr').eq(3).children().eq(2).html(player[1].integral);
		$('tr').eq(3).children().eq(3).html(player[2].integral);
